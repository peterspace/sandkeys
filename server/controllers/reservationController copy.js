...
//not neccessary
const approveReservationByEmail = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
//not neccessary
const rejectedReservationByEmail = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
async function reservationNotificationUser(reservationId) {
  const booking = await Reservation.findById(reservationId).populate('user');
...
module.exports = {
  ...
  approveReservationByEmail,
  rejectedReservationByEmail,
